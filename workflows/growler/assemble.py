...
#: Inputs of `growler.gxwf.yml` that are PARAMETERS rather than data, and must be lifted to every
#: document that wraps it. ⛔ DERIVED FROM THE CHILD, NEVER LISTED HERE. gxformat2 has no include,
#: so a wrapper that forgot to lift one would inline a child whose parameter is simply unconnected
#: -- and an unconnected KegAlign parameter is the defect that made Growler un-invokable in the
#: first place. Reading them off the child means adding an input to Growler propagates by itself.
LIFTED_TYPES = ("text", "integer", "boolean", "float")


def lifted_inputs(growler: dict) -> dict:
    """The child's parameter inputs, in the child's own order, repositioned by the caller."""
    return {name: spec for name, spec in (growler.get("inputs") or {}).items()
            if spec.get("type") in LIFTED_TYPES}
...
def build(growler: dict) -> dict:
    ...
    return {
        ...
        "inputs": {
            ...
            # ⚠ EVERY PARAMETER OF THE CHILD IS LIFTED, not just output_format, so that ONE
            # setting governs the whole panel. A panel whose pairs disagreed about their seed or
            # their hspthresh would produce a collection nobody could describe in a methods section.
            **{n: {**spec, "position": {"top": 370 + 110 * i, "left": 0}}
               for i, (n, spec) in enumerate(lifted_inputs(growler).items())},
        },
        "steps": {
            "pairs": {
            ...
            "growler": {
                ...
                "in": {"target_fasta": "targets/output_filtered",
                       ...
                       "scores": "scores",
                       **{n: n for n in lifted_inputs(growler)}},
...
def _sizes(target: str, query: str) -> tuple[dict, dict]:
    """`(in, state)` for a tool taking two .sizes files from the history.

    ⛔ RETURNS FRESH DICTS ON EVERY CALL, AND THE CALLERS MUST NOT SHARE ONE. `chainprenet` and
    `chainnet` were both assigned the SAME `state` object, so setting a parameter on one silently
    set it on the other. Found 2026-09-24 when a table of 23 defaults landed only 22: `inclHap`
    written for chainPreNet appeared, unasked, in chainNet. Nothing raised -- both tools accept the
    parameter, so the aliased write produced a valid document that simply was not the one intended.
    """
    return (
        {
...
#: Parameters each chain step accepts that nothing else in this document sets. ⛔ GALAXY REFUSES
#: THE WHOLE INVOCATION OVER THEM -- `400 Workflow step has upgrade messages`, one step at a time --
#: so "the tool has a default" is NOT the same as "the step may omit it". This document imported
#: cleanly, passed every offline check, and could not be invoked: measured 2026-09-24, it took five
#: successive 400s to walk from axtChain to chainNet. It had never been hit because Growler had
#: never been run.
#:
#: ⚠ EVERY VALUE HERE IS THE TOOL'S OWN DECLARED DEFAULT, read from /api/tools?io_details=true on
#: usegalaxy.org. Nothing about the computation changes; what changes is that it is stated, which
#: is this repository's rule for tool state anyway.
#:
#: ⚠ `scoreScheme` IS A data PARAMETER and is listed for a reason: Galaxy emits an upgrade message
#: for it too ("Using default: RuntimeValue"), and one message is enough to refuse the invocation.
DEFAULTED: dict[str, dict] = {
    "axtchain": {"minScore": None, "scoreScheme": None, "details_output": False},
    "chainprenet": {"pad": None, "inclHap": False},
    "chainnet": {"minSpace": None, "minFill": None, "minScore": None,
                 "inclHap": False, "verbose": None},
    "netchainsubset": {"splitOnInsert": False, "wholeChains": False, "skipMissing": False},
    "rb_net": {"minSpace": None, "minFill": None, "minScore": None,
               "inclHap": False, "verbose": None},
    "rb_subset": {"splitOnInsert": False, "wholeChains": False, "skipMissing": False},
    "rb_sort1": {"sort_by": ""},
    "rb_sort2": {"sort_by": ""},
}


def apply_defaulted(document: dict) -> int:
    """Fill each step's unnamed parameters from DEFAULTED. Returns how many were set. Pure.

    ⚠ `setdefault`, never overwrite: a value the document already states wins, so adding a
    deliberate setting later cannot be silently reverted by this table.
    """
    n = 0
    for label, params in DEFAULTED.items():
        step = (document.get("steps") or {}).get(label)
        if step is None:
            raise SystemExit(f"DEFAULTED names step `{label}`, which this document does not have")
        state = step.setdefault("state", {})
...
def build_pair(growler: dict) -> dict:
    ...
    """
    # ⚠ the reciprocal-best net runs in the SWAPPED frame, so its two sizes files are the other
    # way round. Each step calls _sizes() for itself -- see that function on why they must not
    # share one dict.
    return {
        "class": "GalaxyWorkflow",
        ...
        "inputs": {
            ...
            # ⚠ Every parameter of the child, lifted, so one setting governs Growler and the
            # chain arm together. `output_format` among them REQUIRES axt here -- axtChain accepts
            # axt,psl only -- which the self-test asserts, because a wrong value fails at the
            # axtChain job rather than at edit time.
            **{n: {**spec, "position": {"top": 550 + 110 * i, "left": 0}}
               for i, (n, spec) in enumerate(lifted_inputs(growler).items())},
        },
        "steps": {
            "growler": {
                ...
                "in": {"target_fasta": "target_fasta", "query_fasta": "query_fasta",
                       "scores": "scores", **{n: n for n in lifted_inputs(growler)}},
            ...
            "chainprenet": {
                ...
                "label": "chainprenet",
                "in": {"in_chain": "chainmergesort/out", **_sizes("target_sizes", "query_sizes")[0]},
                "state": _sizes("target_sizes", "query_sizes")[1],
                "position": {"top": 0, "left": 1200},
            },
            "chainnet": {
                "tool_id": TOOLS["chainnet"],
                "label": "chainnet",
                "in": {"in_chain": "chainprenet/out", **_sizes("target_sizes", "query_sizes")[0]},
                "state": _sizes("target_sizes", "query_sizes")[1],
            ...
            "rb_net": {
                ...
                "in": {"in_chain": "rb_sort1/out", **_sizes("query_sizes", "target_sizes")[0]},
                "state": _sizes("query_sizes", "target_sizes")[1],
...
def render_pair(growler: dict) -> str:
    document = build_pair(growler)
    apply_defaulted(document)
    return BANNER + yaml.safe_dump(document, sort_keys=False, width=100, allow_unicode=True)
...
def self_test() -> int:
    ...
    _doc = build_pair(yaml.safe_load(GROWLER.read_text(encoding="utf-8")))
    _n = apply_defaulted(_doc)
    check("apply_defaulted fills every parameter in the table",
          _n, sum(len(v) for v in DEFAULTED.values()))
    check("  and a second pass is a no-op (setdefault, not overwrite)",
          apply_defaulted(_doc), 0)
    _doc["steps"]["axtchain"]["state"]["minScore"] = 4242
    apply_defaulted(_doc)
    check("  and a value the document already states is NOT reverted",
          _doc["steps"]["axtchain"]["state"]["minScore"], 4242)
